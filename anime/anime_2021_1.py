...
class TomozakiKunDownload(Winter2021AnimeDownload):
    ...
    def download_news(self):
        ...
        try:
            soup = self.get_soup(news_url, decode=True)
            articles = soup.find_all('article', class_='box_main')
            news_obj = self.get_last_news_log_object()
            results = []
    ...
    def download_media(self):
        ...
        self.add_to_image_list('bd_tw_2_1', 'https://pbs.twimg.com/media/EtbxPEtUUAEl-6Z?format=jpg&name=900x900')
        self.add_to_image_list('bd_tw_2_2', 'https://pbs.twimg.com/media/EtbxQTzUUAQocb6?format=jpg&name=large')
        self.add_to_image_list('bd_tw_3_1', 'https://pbs.twimg.com/media/EvIYl6wUYAU7y0B?format=jpg&name=large')
        self.add_to_image_list('bd_tw_3_2', 'https://pbs.twimg.com/media/EvIYmlkUYAY00sg?format=jpg&name=medium')
...
class KakushiDungeonDownload(Winter2021AnimeDownload, NewsTemplate3):
    ...
    def download_news(self):
        self.download_template_news(self.PAGE_PREFIX, 'news/index.html')
    ...
    def download_endcard(self):
        ...
        self.add_to_image_list('ec10', 'https://pbs.twimg.com/media/EwGRazOVgAA2i0f?format=jpg&name=large')
        self.add_to_image_list('ec11', 'https://pbs.twimg.com/media/EwpYWSAVkAYzJdz?format=jpg&name=large')
        self.add_to_image_list('ec12', 'https://pbs.twimg.com/media/ExONY1fVcAEwOXc?format=jpg&name=large')
        self.download_image_list(folder)
        ...
        try:
            ...
            for page in range(1, 100, 1):
                ...
                if page > 1:
                    page_url = news_url + '?p=' + str(page)
                soup = self.get_soup(page_url, decode=True)
                articles = soup.find_all('article', class_='archives__item')
                for article in articles:
                    tag_date = article.find('p', class_='archives__item__date')
                    tag_title = article.find('h2', class_='archives__item__title')
                    ...
                    if tag_date and tag_title and a_tag and a_tag.has_attr('href'):
                        ...
                        if news_obj and (news_obj['id'] == article_id or date < news_obj['date']):
                            ...
                            break
                        results.append(self.create_news_log_object(date, title, article_id))
                if stop or soup.find('a', class_='c-btn-next') is None:
                    break
            success_count = 0
            for result in reversed(results):
                process_result = self.create_news_log_from_news_log_object(result)
