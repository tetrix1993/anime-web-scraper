                    ...
                    if 'img_slide' in content and isinstance(content['img_slide'], list):
                        ...
                        for i in range(len(content['img_slide'])):
                            if 'img' in content['img_slide'][i] and 'url' in content['img_slide'][i]['img']:
                                image_url = content['img_slide'][i]['img']['url'] + '?a'
            ...
            if len(story_list) > 0:
                ...
                for story in reversed(stories):
                    ...
                    if ep_soup is not None:
                        images = ep_soup.select('.thumbnail__cont .swiper-wrapper img[src]')
                        self.image_list = []
                        for i in range(len(images)):
...
class ChainsawManDownload(Fall2022AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        template = self.PAGE_PREFIX + 'episodes/img/%s.jpg'
        for i in range(self.FINAL_EPISODE):
            episode = str(i + 1).zfill(2)
...
class KanColle2Download(Fall2022AnimeDownload, NewsTemplate2):
    ...
    keywords = [title, 'Kantai Collection', 'KanColle Season 2: Let\'s Meet at Sea']
        ...
        try:
            ...
            for a_tag in a_tags:
                ...
                if chara_soup is None:
                    continue
                self.image_list = []
                images = chara_soup.select('.chara__stand img[src],.chara__face img[src]')
                for image in images:
                    if '/chara/' not in image['src']:
                        continue
                    image_url = self.PAGE_PREFIX + image['src'].replace('../', '')
...
class GundamWitchDownload(Fall2022AnimeDownload):
    ...
    def download_media(self):
        folder = self.create_media_directory()
        pages = ['benefit'] + [str(x + 1) for x in range(8)]
        cache_filepath = folder + '/cache'
        processed, num_processed = self.get_processed_items_from_cache_file(cache_filepath)
        for page in pages:
            bd_url = self.PAGE_PREFIX + 'bddvd/' + page + '/'
            try:
                soup = self.get_soup(bd_url)
                self.image_list = []
                if page.isnumeric():
                    if page in processed:
                        continue
                    images = soup.select('.detail__images p[style]')
                    for image in images:
                        if "background-image: url('" in image['style'] and image['style'].endswith("')"):
                            image_url = self.PAGE_PREFIX + image['style'][23:].replace('../', '').split("')")[0]
                            image_name = self.extract_image_name_from_url(image_url)
                            self.add_to_image_list(image_name, image_url)
                    ...
                    images = soup.select('.benefit img[src]')
                    for image in images:
                        if 'icon_printing' not in image['src']:
                            image_url = self.PAGE_PREFIX + image['src'].replace('../', '')
                            image_name = self.extract_image_name_from_url(image_url)
                            self.add_to_image_list(image_name, image_url)
                    self.download_image_list(folder)
            except Exception as e:
                self.print_exception(e, 'Blu-ray %s' % page)
        self.create_cache_file(cache_filepath, processed, num_processed)
...
class NouminKanrenDownload(Fall2022AnimeDownload, NewsTemplate):
    ...
    def download_media(self):
        ...
        try:
            ...
            images = soup.select('.subpage-Body img[src]')
            self.image_list = []
            for image in images:
                image_url = image['src']
                image_name = self.extract_image_name_from_url(image_url)
...
class RenaiFlopsDownload(Fall2022AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview_external(self):
        ...
        AniverseMagazineScanner(keywords, self.base_folder, last_episode=self.FINAL_EPISODE,
                                end_date='20221010', download_id=self.download_id).run()
...
class ShinmaiRenkinDownload(Fall2022AnimeDownload, NewsTemplate2):
    ...
    def download_episode_preview_guess(self):
        ...
        for i in range(6, self.FINAL_EPISODE + 1, 1):
            ...
            first = 22 + (i - 6)
            second = 65 + 5 * (i - 6)
            third = 65 + self.IMAGES_PER_EPISODE * (i - 6)
            for j in range(self.IMAGES_PER_EPISODE):
                image_url = template % (str(first).zfill(8), str(second).zfill(8), str(third + j).zfill(8))
                image_name = episode + '_' + str(j + 1)
                result = self.download_image(image_url, folder + '/' + image_name)
...
class YamaNoSusume4Download(Fall2022AnimeDownload):
    ...
    def download_episode_preview_guess(self):
        ...
        template = self.PAGE_PREFIX + 'core_sys/images/contents/%s/block/%s/%s.jpg'
        is_successful = False
        for i in range(7, self.FINAL_EPISODE + 1, 1):
            episode = str(i).zfill(2)
            is_success = False
            if self.is_image_exists(episode + '_1'):
                continue
            ...
            for j in range(self.IMAGES_PER_EPISODE):
                ...
                image_url = template % (str(first).zfill(8), str(second_).zfill(8), str(third + j).zfill(8))
                image_name = episode + '_' + str(j + 1)
                result = self.download_image(image_url, folder + '/' + image_name)
                if result == 0:
                    is_success = True
