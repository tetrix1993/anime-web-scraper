        ...
        for i in range(self.FIRST_EPISODE, self.FINAL_EPISODE + 1, 1):
            ...
            if download_valid and len(valid_urls) > 0:
                for m in range(len(valid_urls)):
                    ...
                    self.download_image(valid_urls[m], folder + '/' + image_name, to_jpg=True)
            if not episode_success:
                break
            is_successful = True
        if is_successful:
...
class LiarLiarDownload(Summer2023AnimeDownload, NewsTemplate):
    ...
    def download_media(self):
        ...
        try:
            ...
            elems = soup.select('#ll_contents_staffcast_music img[src],' +
                                '#ll_contents_staffcast_music span[style^="background-image:url(\'"]')
            self.image_list = []
            for elem in elems:
                if elem.name == 'img':
                    image_url = elem['src']
                    ...
                    try:
                        image_url = elem['style'].split("background-image:url('")[1].split("'")[0]
                    except:
                        continue
                if '/blu-ray/' not in image_url:
                    continue
                image_name = self.generate_image_name_from_url(image_url, 'blu-ray')
                self.add_to_image_list(image_name, image_url)
            self.download_image_list(folder)
    ...
    def download_episode_preview_guess(self):
        ...
        for i in range(self.FIRST_EPISODE, self.FINAL_EPISODE + 1):
            ...
            if self.is_image_exists(episode + '_1'):
                continue
            is_success = False
            first = 59 + i - self.FIRST_EPISODE
            second = 225 + 4 * (i - self.FIRST_EPISODE)
            third = 442 + self.IMAGES_PER_EPISODE * (i - self.FIRST_EPISODE)
            for j in range(self.IMAGES_PER_EPISODE):
                image_url = template % (str(first).zfill(8), str(second).zfill(8), str(third + j).zfill(8))
                # if not self.is_content_length_in_range(image_url, more_than_amount=7000):
                #     break
...
class ShiroSeijoDownload(Summer2023AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        ...
        try:
            ...
            story_list = soup.select('.storyTabLists')
            if len(story_list) > 0:
                lis = story_list[0].select('li')
                for li in lis:
                    a_tag = li.select('a[href]')
                    try:
                        ep_num = int(a_tag[0].select('.storyTabList_txt')[0].text.replace('第', '').replace('話', ''))
                        if ep_num is None or ep_num < 1:
                            continue
                        episode = str(ep_num).zfill(2)
                    except:
                    ...
                    if self.is_image_exists(episode + '_1'):
                        continue
                    if a_tag[0].has_attr('class') and '--is-active' in li['class']:
                        ep_soup = soup
                    else:
                        ep_soup = self.get_soup(story_url + a_tag[0]['href'].replace('./', ''))
                    if ep_soup is not None:
