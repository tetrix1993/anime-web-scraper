...
class AtriDownload(Summer2024AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            ...
            stories = soup.select('.story__nav__item')
            for story in stories:
                try:
                ...
                if ep_soup is None:
                    continue
                self.image_list = []
                images = ep_soup.select('.thumbnail__main .swiper-wrapper img[src]')
                for i in range(len(images)):
                    image_name = episode + '_' + str(i + 1)
...
class IseyuruDownload(Summer2024AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            template = self.PAGE_PREFIX + 'dist/img/top/story/ep%s/%s.jpg'
            stop = False
            for i in range(self.FINAL_EPISODE):
                episode = str(i + 1).zfill(2)
                if self.is_image_exists(episode + '_' + str(self.IMAGES_PER_EPISODE)):
...
class Kimisen2Download(Summer2024AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            template = self.PAGE_PREFIX + 'assets/story/s2/%s/%s.webp'
            stop = False
            for i in range(self.FINAL_EPISODE):
                episode = str(i + 1).zfill(2)
                if self.is_image_exists(episode + '_' + str(self.IMAGES_PER_EPISODE)):
...
class FutakireDownload(Summer2024AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview_external(self):
        keywords = ['恋は双子で割り切れない']
        AniverseMagazineScanner(keywords, self.base_folder, last_episode=self.FINAL_EPISODE,
                                end_date='20240703', download_id=self.download_id).run()
...
class KonofukaDownload(Summer2024AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            ...
            for story in stories:
                try:
                    episode = str(int(story.select('.story-episode-ttl')[0].text
                                      .split(' ')[0].replace('No.', ''))).zfill(2)
                except:
                    continue
                if self.is_image_exists(episode + '_1'):
                    continue
                ...
                if ep_soup is None:
                    continue
                images = ep_soup.select('.story-thumb-list img[src]')
                self.image_list = []
                for i in range(len(images)):
                    image_url = images[i]['src']
                    image_name = episode + '_' + str(i + 1)
...
class MakeineDownload(Summer2024AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            story_prefix = self.PAGE_PREFIX + 'story/'
            soup = self.get_soup(story_prefix, decode=True)
            stories = soup.select('.story_nav__item')
            for story in stories:
                try:
                    episode = str(int(story.text.strip().replace('第', '').replace('話', ''))).zfill(2)
                except:
                    continue
                if self.is_image_exists(episode + '_1'):
                    continue
...
class MayopanDownload(Summer2024AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            story_prefix = self.PAGE_PREFIX + 'story/'
            soup = self.get_soup(story_prefix, decode=True)
            stories = soup.select('.storyList a[href]')
            for story in stories:
                try:
                    episode = str(int(story.select('.storyList__link_txt__num')[0].text.replace('#', ''))).zfill(2)
                except:
                    continue
                if self.is_image_exists(episode + '_1'):
                    continue
                if '--is-active' in story['class']:
                    ...
                    story_url = story_prefix + story['href']
                    ep_soup = self.get_soup(story_url)
                if ep_soup is None:
                    continue
                self.image_list = []
                images = ep_soup.select('.storyImageList__img img[src]')
                for i in range(len(images)):
                    image_name = episode + '_' + str(i + 1)
                    image_url = story_prefix + images[i]['src'].replace('./', '')
                    self.add_to_image_list(image_name, image_url)
                self.download_image_list(self.base_folder)
        except Exception as e:
            self.print_exception(e)
    ...
    def download_episode_preview_guess(self, print_invalid=False, download_valid=True):
        ...
        for i in range(self.FINAL_EPISODE):
            ...
            if self.is_image_exists(episode + '_1'):
                continue
            for j in range(self.IMAGES_PER_EPISODE):
                image_name = f'parry_{episode}-{j+1}'
                if self.is_image_exists(image_name, folder):
                    continue
                image_url = self.PAGE_PREFIX + f'wp/wp-content/uploads/{year}/{month}/{image_name}.jpg'
                if self.is_valid_url(image_url, is_image=True):
                    print('VALID - ' + image_url)
                    is_successful = True
                    valid_urls.append({'name': image_name, 'url': image_url, 'folder': folder})
                else:
                    if print_invalid:
                        print('INVALID - ' + image_url)
                    stop = True
...
class WistoriaDownload(Summer2024AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            template = self.PAGE_PREFIX + 'jrepgmrf/wp-content/themes/wistoria-anime/assets/img/episodes/ep%s/%s.jpg'
            stop = False
            for i in range(self.FINAL_EPISODE):
                episode = str(i + 1).zfill(2)
                if self.is_image_exists(episode + '_' + str(self.IMAGES_PER_EPISODE)):
