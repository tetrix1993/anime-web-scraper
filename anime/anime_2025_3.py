...
class NineDownload(Summer2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            ...
            stories = soup.select('.story-article[id]')
            for story in stories:
                ...
                if self.is_image_exists(episode + '_1'):
                    continue
                images = story.select('.story-img-slide__item img[src]')
                self.image_list = []
...
class BusagachiDownload(Summer2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            ...
            stories = soup.select('.episode__number-item a[href]')
            for story in stories:
                if not story['href'].startswith('#'):
                    continue
                try:
                    episode = ''
                    ep_num = story.text
                ...
                if self.is_image_exists(episode + '_1'):
                    continue
                images = soup.select(story['href'] + ' .episode__contents-sliderItem-wrap img[src]')
                self.image_list = []
                for i in range(len(images)):
...
class FoodCourtDownload(Summer2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            soup = self.get_soup(self.PAGE_PREFIX + 'story/')
            stories = soup.select('.story-Detail_Item[id]')
            for story in stories:
                try:
                    episode = ''
                    ep_num = story['id']
                ...
                if self.is_image_exists(episode + '_1'):
                    continue
                images = story.select('.swiper-wrapper img[data-src]')
                for i in range(len(images)):
...
class MynoghraDownload(Summer2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        template = self.PAGE_PREFIX + 'dist/img/story/episode/ep%s/img%s.webp'
        try:
            stop = False
            for i in range(self.FINAL_EPISODE):
                episode = str(i + 1).zfill(2)
    ...
    def download_episode_preview_external(self):
        keywords = ['異世界黙示録マイノグーラ']
        AniverseMagazineScanner(keywords, self.base_folder, last_episode=self.FINAL_EPISODE,
                                end_date='20250629', download_id=self.download_id).run()
...
class MattanDownload(Summer2025AnimeDownload, NewsTemplate2):
    ...
    def download_episode_preview_external(self):
        keywords = ['まったく最近の探偵ときたら']
        AniverseMagazineScanner(keywords, self.base_folder, last_episode=self.FINAL_EPISODE,
                                end_date='20250627', download_id=self.download_id).run()
    ...
    def download_episode_preview(self):
        ...
        try:
            ...
            for story in stories:
                try:
                    episode = ''
                    ep_num = story.select('.p-story_nav__link-text')[0].text
                    for a in ep_num:
                        if a.isnumeric():
                            episode += a
                    if len(episode) == 0:
                ...
                if 'is-current' in story['class']:
                    ...
                    try:
                        ep_soup = self.get_soup(self.PAGE_PREFIX + story.select('a[href]')[0]['href'][1:])
                    except:
                        continue
                if ep_soup is None:
                    continue
                self.image_list = []
                images = ep_soup.select('.p-story_visual__img img[src]')
                for i in range(len(images)):
                    image_url = prefix + images[i]['src']
                    image_name = episode + '_' + str(i + 1)
                    self.add_to_image_list(image_name, image_url, to_jpg=True)
...
class AobutaSantaDownload(Summer2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        ...
        try:
            soup = self.get_soup(prefix)
            stories = soup.select('li.p-story__contents-nav_item')
            for story in stories:
                try:
                    episode = ''
                    ep_num = story.select('a')[0].text
                    for a in ep_num:
                        if a.isnumeric():
                            episode += a
                    if len(episode) == 0:
...
class Kisekoi2Download(Summer2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        ...
        try:
            soup = self.get_soup(prefix)
            stories = soup.select('.p-story__nav-list-item')
            for story in stories:
                try:
                    episode = ''
                    ep_num = story.select('.p-in-text')[0].text
                    for a in ep_num:
                        if a.isnumeric():
                            episode += a
                    if len(episode) == 0:
                ...
                if ep_soup is None:
                    continue
                self.image_list = []
                images = ep_soup.select('.p-story__main-image-list-item img[src]')
                for i in range(len(images)):
                    image_url = prefix + images[i]['src']
                    image_name = episode + '_' + str(i + 1)
                    self.add_to_image_list(image_name, image_url, to_jpg=True)
...
class TateNoYuusha4Download(Summer2025AnimeDownload):
    ...
    def download_episode_preview(self):
        template = self.PAGE_PREFIX + 'assets/img/4th/story/ss/ep%s/%s.webp'
        try:
            stop = False
            for i in range(self.FINAL_EPISODE):
                episode = str(i + 1).zfill(2)
...
class TsuyosagaDownload(Summer2025AnimeDownload, NewsTemplate4):
    ...
    def download_episode_preview_guess(self, print_url=False, print_invalid=False):
        if self.is_image_exists(str(self.FINAL_EPISODE).zfill(2) + '_1'):
            return
        folder = self.create_custom_directory('guess')
        template = self.PAGE_PREFIX + 'wp/wp-content/uploads/%s/%s/tsuyosaga_%s-%s.jpg'
        current_date = datetime.now() + timedelta(hours=1)
        year = current_date.strftime('%Y')
        month = current_date.strftime('%m')
        is_successful = False
...
class WatanareDownload(Summer2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        ...
        try:
            soup = self.get_soup(prefix)
            stories = soup.select('.link__item')
            for story in stories:
                try:
                    episode = ''
                    ep_num = story.select('a[href]')[0].text
                    for a in ep_num:
                        if a.isnumeric():
                            episode += a
                    if len(episode) == 0:
                ...
                if '--current' in story['class']:
                    ...
                    try:
                        ep_soup = self.get_soup(prefix + story.select('a[href]')[0]['href'])
                    except:
                        continue
                if ep_soup is None:
                    continue
                self.image_list = []
                images = ep_soup.select('.story-img-slide__item img[src]')
                for i in range(len(images)):
