...
class HojomahoDownload(Fall2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            soup = self.get_soup(self.PAGE_PREFIX + 'episode/')
            stories = soup.select('a.newsCol[href]')
            for story in stories:
                try:
                    episode = ''
                    ep_num = story.select('.newsCol__title')[0].text
                    is_num = False
                    for a in ep_num:
                        if a.isnumeric():
                            episode += a
...
class OnePunchMan3Download(Fall2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            stories = self.get_json(self.PAGE_PREFIX + 'news/episode_data')
            for story in stories:
                try:
                    episode_int = int(story['acf']['id'])
    ...
    def download_episode_preview(self):
        try:
            ...
            for story in stories:
                episode = ''
                try:
                    ep_num = story.select('.story_num')[0].text
                    is_num = False
                    for a in ep_num:
                        if a.isnumeric():
                            episode += a
                ...
                if self.is_image_exists(episode + '_1'):
                    continue
                images = story.select('.story_body_thumb a[data-imgload]')
                for i in range(len(images)):
...
class TondemoSkill2Download(Fall2025AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            soup = self.get_soup(self.PAGE_PREFIX + 'story/')
            stories = soup.select('.story__item a[href]')
            for story in stories:
                try:
                    episode = ''
                    ep_num = story.select('.story__num')[0].text
                    is_num = False
                    for a in ep_num:
                        if a.isnumeric():
                            episode += a
                ...
                if self.is_image_exists(episode + '_1'):
                    continue
                ep_soup = self.get_soup(story['href'])
                images = ep_soup.select('.storyDetail__item img[src]')
                for i in range(len(images)):
                    image_url = images[i]['src']
                    image_name = episode + '_' + str(i + 1)
                    self.add_to_image_list(image_name, image_url, to_jpg=True)
