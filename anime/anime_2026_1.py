...
class ChampignonDownload(Winter2026AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview_guess(self, print_url=False, print_invalid=False, download_valid=True):
        ...
        template = self.PAGE_PREFIX + 'wp/wp-content/uploads/%s/%s/Champignon_ep%s-%s.'
        extensions = ['jpg']
...
class OkirakuRyoushuDownload(Winter2026AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            soup = self.get_soup(self.PAGE_PREFIX + 'story/')
            stories = soup.select('.bl_episode__content[id]')
            for story in stories:
                try:
                    episode = ''
                    ep_num = story['id']
                ...
                if self.is_image_exists(episode + '_1'):
                    continue
                images = story.select('.bl_episode__imgMain_item img[src]')
                self.image_list = []
                for i in range(len(images)):
...
# Oshi no Ko Season 3
class Oshinoko3Download(Winter2026AnimeDownload, NewsTemplate2):
    title = 'Oshi no Ko Season 3'
    keywords = [title, 'oshinoko']
    website = 'https://ichigoproduction.com/Season3/'
    twitter = 'anime_oshinoko'
    hashtags = '推しの子'
    folder_name = 'oshinoko3'
...
class YushanoKuzuDownload(Winter2026AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        template = self.PAGE_PREFIX + 'dist/img/episode/ep%s/img%s.webp'
        try:
            stop = False
            for i in range(self.FINAL_EPISODE):
                episode = str(i + 1).zfill(2)
