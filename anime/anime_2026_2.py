    ...
    def download_episode_preview(self):
        try:
            ...
            for btn in btns:
                ...
                if self.is_image_exists(episode + '_1'):
                    continue
                panel = soup.select('#' + btn['data-story'])
                if len(panel) == 0:
                    continue
                images = panel[0].select('div[id^="main-slider"] img[src]')
                for i in range(len(images)):
                    image_url = images[i]['src']
                    image_name = episode + '_' + str(i + 1)
                    self.add_to_image_list(image_name, image_url, to_jpg=True)
...
class NonbiriNouka2Download(Spring2026AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview_guess(self, print_invalid=False, download_valid=True):
        ...
        folder = self.create_custom_directory('guess')
        template = self.PAGE_PREFIX + 'wp/wp-content/uploads/%s/%s/nonbirinouka2_ep%s_cap-%s_1.jpg'
        current_date = datetime.now() + timedelta(hours=1)
        year = current_date.strftime('%Y')
        month = current_date.strftime('%m')
        is_successful = False
        ...
        for i in range(self.FINAL_EPISODE):
            ...
            j = 0
            img_num = 0
            while j < 200 and img_num < self.IMAGES_PER_EPISODE:
                j += 1
                image_url = template % (year, month, episode, str(j).zfill(3))
                if self.is_valid_url(image_url, is_image=True):
                    print('VALID - ' + image_url)
                    image_name = episode + '_' + str(img_num + 1)
                    is_successful = True
                    is_success = True
                    img_num += 1
    ...
    def download_episode_preview_guess(self, print_invalid=False, download_valid=True):
        ...
        for i in range(self.FINAL_EPISODE):
            ...
            for j in range(200):
                ...
                image_name = episode + '_' + str(img_count + 1)
                if self.is_valid_url(image_url, is_image=True):
                    print('VALID - ' + image_url)
                    is_successful = True
                    is_success = True
                    ...
                    if print_invalid:
                        print('INVALID - ' + image_url)
                if img_count == self.IMAGES_PER_EPISODE or (img_count == 0 and j == 50):
                    break
            if not is_success:
                break
        if download_valid and len(valid_urls) > 0:
    ...
    def download_episode_preview(self):
        try:
            ...
            for a_tag in a_tags:
                ...
                if self.is_image_exists(episode + '_1'):
                    continue
                if 'is-Current' in a_tag['class']:
                    ep_soup = soup
                else:
                    ep_soup = self.get_soup(a_tag['href'])
                if ep_soup is None:
                    continue
                images = ep_soup.select('.p-story__sceneImage-item img[src]')
                for i in range(len(images)):
                    image_url = images[i]['src']
                    image_name = episode + '_' + str(i + 1)
                    self.add_to_image_list(image_name, image_url, to_jpg=True)
...
class OtagalDownload(Spring2026AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview_guess(self, print_invalid=False, download_valid=True):
        ...
        folder = self.create_custom_directory('guess')
        template = self.PAGE_PREFIX + 'wp/wp-content/uploads/%s/%s/【宣伝使用】OG__%s_%s.png'
        current_date = datetime.now() + timedelta(hours=1)
        year = current_date.strftime('%Y')
        month = current_date.strftime('%m')
        is_successful = False
        ...
        for i in range(self.FINAL_EPISODE):
            ...
            while j < 200 and img_num < self.IMAGES_PER_EPISODE:
                j += 1
                image_url = template % (year, month, episode, str(j).zfill(6))
                if self.is_valid_url(image_url, is_image=True):
                    print('VALID - ' + image_url)
                    image_name = episode + '_' + str(img_num + 1)
                    is_successful = True
