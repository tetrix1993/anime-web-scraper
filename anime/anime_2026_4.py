...
class MaryokuRevoDownload(Fall2026AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        try:
            ...
            stories = soup.select('.story-list a[href]')
            for story in stories:
                try:
                    episode = ''
                    ep_num = story.select('.story-ep')[0].text
                    for a in ep_num:
                        if a.isnumeric():
                            episode += a
                    if len(episode) == 0:
                ...
                if ep_soup is None:
                    continue
                self.image_list = []
                images = ep_soup.select('#story-main .splide__slide img[src]')
...
class MajiekuDownload(Fall2026AnimeDownload, NewsTemplate):
    ...
    def download_episode_preview(self):
        episode_url = self.PAGE_PREFIX + 'episodes/'
        try:
            soup = self.get_soup(episode_url, decode=True)
            stories = soup.select('.story__navList a[href]')
            for story in stories:
                try:
                    episode = ''
                    ep_num = story.text
                ...
                if '--is-current' in story['class']:
                    ...
                    ep_soup = self.get_soup(episode_url + story['href'])
                if ep_soup is None:
                    continue
                self.image_list = []
                images = ep_soup.select('.story__imageList img[src]')
                for i in range(len(images)):
...
class Tanmoshi2Download(Fall2026AnimeDownload, NewsTemplate2):
    title = 'Tantei wa Mou, Shindeiru. Season 2'
    keywords = [title, 'The Detective Is Already Dead Season 2', 'tanmoshi', '2nd']
    website = 'https://tanmoshi-anime.jp/Season2/'
    twitter = 'tanteiwamou_'
    hashtags = ['たんもし', 'tanmoshi']
    folder_name = 'tanmoshi2'
